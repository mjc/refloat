...
static bool test_balance_filter_nonfinite_dt(void) {
    ...
    RED_EXPECT_TRUE(isfinite(bf.q0));
    RED_EXPECT_TRUE(isfinite(bf.q1));
    RED_EXPECT_TRUE(isfinite(bf.q2));
    RED_EXPECT_TRUE(isfinite(bf.q3));
    RED_EXPECT_TRUE(isfinite(balance_filter_get_roll(&bf)));
    RED_EXPECT_TRUE(isfinite(balance_filter_get_pitch(&bf)));
    RED_EXPECT_TRUE(isfinite(balance_filter_get_yaw(&bf)));

    float norm = sqrtf(bf.q0 * bf.q0 + bf.q1 * bf.q1 + bf.q2 * bf.q2 + bf.q3 * bf.q3);
    RED_EXPECT_TRUE(fabsf(norm - 1.0f) < 0.00001f);
