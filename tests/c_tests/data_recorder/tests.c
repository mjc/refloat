...
static bool test_data_recorder_experiment_plot_export(void) {
    ...
    fixture.data.atr.transition_boost = 7.25f;
    ...
    EXPECT_FLOAT_NEAR(vesc_if_fake_last_plot_x(), 222.0f / SYSTEM_TICK_RATE_HZ);
    EXPECT_FLOAT_NEAR_EPS(vesc_if_fake_last_plot_y(), 7.25f, 0.01f);

    return true;
}

static bool test_data_recorder_validates_request_range(void) {
    vesc_if_fake_reset();
    ...
    EXPECT_TRUE(!fixture.dr.recording);
    data_recorder_send_experiment_plot(&fixture.dr);
    EXPECT_EQ_U32(vesc_if_fake_plot_init_calls(), 0u);
    ...
    vesc_if_fake_reset();
    EXPECT_TRUE(SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 1, 1) == NULL);
    EXPECT_EQ_U32(len, 0u);
    EXPECT_TRUE(SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 2, 1) == NULL);
    EXPECT_EQ_U32(len, 0u);
    ...
    payload = SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 1, 99, 1);
    EXPECT_TRUE(payload != NULL);
    EXPECT_EQ_U32(payload[1], 41u);
    EXPECT_TRUE(fixture.dr.autostart);
    EXPECT_TRUE(fixture.dr.autostop);
    ...
    vesc_if_fake_reset();
    EXPECT_TRUE(SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 0, 0) == NULL);
    EXPECT_TRUE(SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 1, 1) == NULL);
    EXPECT_TRUE(SEND_DATA_RECORDER_REQUEST(&fixture.dr, len, 2, 99) == NULL);

    fixture.dr.autostart = false;
...
static bool test_data_recorder_sample_rate_recomputes_decimation(void) {
    ...
    fixture.dr.sample_count = 1;
    data_recorder_set_sample_rate(&fixture.dr, UINT16_MAX);
    EXPECT_EQ_U32(fixture.dr.decimation, UINT8_MAX);

    return true;
}

static bool test_data_recorder_zero_sample_rate_status_is_defined(void) {
    vesc_if_fake_reset();
    Sample storage[1] = {0};
    DataRecorderFixture fixture;
    data_recorder_fixture_start(&fixture, storage, 1, true, false, false, false, 1, 0, 1);
...
static bool test_data_recorder_data_payload_is_bounded(void) {
    vesc_if_fake_reset();

    Sample storage[32] = {0};
    DataRecorderFixture fixture;
    data_recorder_fixture_start(&fixture, storage, 32, true, true, false, false, 1, 1000, 32);
    fixture.data.state.state = STATE_RUNNING;

    for (size_t sample_index = 0; sample_index < 32; ++sample_index) {
        fixture.data.motor.erpm = (float) sample_index;
        data_recorder_sample(&fixture.dr, &fixture.data, (time_t) (sample_index + 1));
    }
    ...
    EXPECT_TRUE(payload != NULL);
    EXPECT_TRUE(len <= SEND_BUF_MAX_SIZE);
    EXPECT_TRUE(len >= 6u);
    EXPECT_EQ_U32((len - 6u) % sample_size, 0u);
    EXPECT_EQ_U32((len - 6u) / sample_size, (SEND_BUF_MAX_SIZE - 6u) / sample_size);
    EXPECT_EQ_U32(read_be32(&payload[2]), 0u);
...
static bool test_data_recorder_rejects_incompatible_magic(void) {
    vesc_if_fake_reset();

    uint8_t storage[sizeof(Sample)] = {0};
    const uint32_t incompatible_magic[] = {0u, 0x12345678u, 0xcafe1021u, 0xcafe1010u};
    for (size_t i = 0; i < sizeof(incompatible_magic) / sizeof(*incompatible_magic); ++i) {
        DataRecord dr = {0};
        vesc_if_fake_set_data_buffer(incompatible_magic[i], storage, sizeof(storage));
        data_recorder_init(&dr, 100u);

        EXPECT_TRUE(!data_recorder_has_capability(&dr));
        EXPECT_EQ_U32(dr.sample_count, 0u);
    }
    return true;
}

static bool test_data_recorder_initializes_valid_buffer(void) {
    vesc_if_fake_reset();

    DataRecord dr = {0};
    Sample storage[2] = {0};
    vesc_if_fake_set_data_buffer(0xcafe1011u, (uint8_t *) storage, sizeof(storage));
    data_recorder_init(&dr, 20u);

    EXPECT_TRUE(data_recorder_has_capability(&dr));
    EXPECT_EQ_U32(dr.sample_count, 2u);
    EXPECT_EQ_U32(dr.sample_rate, 20u);
    EXPECT_EQ_U32(dr.decimation, 100u);
    EXPECT_EQ_U32(circular_buffer_size(&dr.buffer), 0u);
    return true;
}

static bool test_data_recorder_makes_timestamps_strictly_increasing(void) {
    vesc_if_fake_reset();

    Sample storage[2] = {0};
    DataRecorderFixture fixture;
    data_recorder_fixture_start(&fixture, storage, 2, true, true, false, false, 1, 100, 2);
    data_recorder_sample(&fixture.dr, &fixture.data, 100u);
    data_recorder_sample(&fixture.dr, &fixture.data, 100u);

    Sample first = {0};
    Sample second = {0};
    EXPECT_TRUE(circular_buffer_get(&fixture.dr.buffer, 0, &first));
    EXPECT_TRUE(circular_buffer_get(&fixture.dr.buffer, 1, &second));
    EXPECT_EQ_U32(first.time, 100u);
    EXPECT_EQ_U32(second.time, 101u);
    return true;
}
