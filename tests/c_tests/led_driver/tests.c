...
static bool test_led_driver_setup_and_color_encoding(void) {
    ...
    uint32_t status_data[] = {0x000000FFu};
    ...
    const LedStrip *strips[STRIP_COUNT] = {&front, &rear, &status};
    ...
    EXPECT_TRUE(fixture.driver.bitbuffer != NULL);
    EXPECT_TRUE(
        fixture.driver.bitbuffer_length ==
        24u * front.length + 32u * rear.length + 24u * status.length + 1u
    );
    ...
    EXPECT_TRUE(
        fixture.driver.strip_bitbuffs[2] ==
        fixture.driver.bitbuffer + 24u * front.length + 32u * rear.length
    );
...
static bool test_led_driver_rejects_invalid_pin(void) {
    ...
    EXPECT_TRUE(!led_driver_setup(
        &fixture.driver, (LedPin) (LED_PIN_LAST + 1), LED_PIN_CFG_NO_PULLUP, strips
    ));
    EXPECT_TRUE(fixture.driver.bitbuffer == NULL);
    EXPECT_TRUE(fixture.driver.bitbuffer_length == 0);
...
static bool test_led_driver_accepts_strip_above_legacy_limit(void) {
    LedDriverFixture fixture;
    led_driver_fixture_start(&fixture);
    ...
    EXPECT_TRUE(led_driver_setup(&fixture.driver, LED_PIN_B6, LED_PIN_CFG_NO_PULLUP, strips));
    EXPECT_EQ_U32(fixture.driver.bitbuffer_length, 24u * 31u + 1u);
    led_driver_fixture_destroy(&fixture);

    return true;
}

static bool test_led_driver_setup_fails_when_malloc_fails(void) {
    LedDriverFixture fixture;
    led_driver_fixture_start(&fixture);
    ...
    EXPECT_TRUE(!led_driver_setup(&fixture.driver, LED_PIN_B6, LED_PIN_CFG_NO_PULLUP, strips));
    EXPECT_TRUE(fixture.driver.bitbuffer == NULL);
    EXPECT_EQ_U32(fixture.driver.bitbuffer_length, 25u);
...
static bool test_led_driver_full_brightness_color_orders(void) {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        for (uint8_t byte = 0; byte < cases[i].bit_count / 8u; ++byte) {
            EXPECT_TRUE(expect_byte_bits_msb(
                fixture.driver.bitbuffer, byte * 8u, cases[i].bytes[byte], zero, one
            ));
        }
