...
static bool test_main_lights_control_protocol(void) {
    ...
    index = 2;
    buffer_append_uint32(set_lights, 0x00000001u, &index);
    set_lights[index++] = 0x01u;
    vesc_if_fake_invoke_app_data_handler(set_lights, sizeof(set_lights));
...
static bool test_main_alerts_protocol(void) {
    ...
    d->time.now = 1234u;
    alert_tracker_add(&d->alert_tracker, &d->time, ALERT_FW_FAULT, FAULT_CODE_ABS_OVER_CURRENT);
    ...
    EXPECT_EQ_U32(payload[index++], FAULT_CODE_ABS_OVER_CURRENT);
    static const char fault_name[] = "ABS_OVER_CURRENT";
    EXPECT_EQ_U32(payload[index++], sizeof(fault_name) - 1);
    EXPECT_MEM_EQ(&payload[index], fault_name, sizeof(fault_name) - 1);
    index += sizeof(fault_name) - 1;
    ...
    d->alert_tracker.fw_fault_code = FAULT_CODE_NONE;
    ...
    circular_buffer_clear(&d->alert_tracker.alert_buffer);
    AlertRecord wrapped = {
        .time = 2u,
        .id = ALERT_FW_FAULT,
        .active = true,
    ...
    circular_buffer_push(&d->alert_tracker.alert_buffer, &wrapped);
    index = 2;
    buffer_append_uint32(filtered_request, UINT32_MAX - 2u, &index);
    vesc_if_fake_invoke_app_data_handler(filtered_request, sizeof(filtered_request));
    payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    EXPECT_EQ_U32(payload[11], 1u);
    index = 12;
    EXPECT_EQ_U32(buffer_get_uint32(payload, &index), 2u);

    wrapped.time = UINT32_MAX / 2u + 1u;
    circular_buffer_clear(&d->alert_tracker.alert_buffer);
    circular_buffer_push(&d->alert_tracker.alert_buffer, &wrapped);
    uint8_t all_request[] = {101, MAIN_COMMAND_ALERTS_LIST};
    vesc_if_fake_invoke_app_data_handler(all_request, sizeof(all_request));
    payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    EXPECT_EQ_U32(payload[11], 1u);

    circular_buffer_clear(&d->alert_tracker.alert_buffer);
    wrapped.code = FAULT_CODE_ABS_OVER_CURRENT;
    for (uint8_t i = 0; i < ALERT_TRACKER_SIZE; ++i) {
        wrapped.time = i + 1u;
        circular_buffer_push(&d->alert_tracker.alert_buffer, &wrapped);
    }
    vesc_if_fake_invoke_app_data_handler(all_request, sizeof(all_request));
    payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    EXPECT_TRUE(payload[11] < ALERT_TRACKER_SIZE);
...
static bool test_main_invalid_command_protocol(void) {
    ...
    vesc_if_fake_set_output_disabled(true);
    vesc_if_fake_invoke_app_data_handler(unknown_command, sizeof(unknown_command));
...
static bool test_main_info_rejects_unknown_led_mode(void) {
    MainProtocolFixture fixture = {0};
    EXPECT_TRUE(main_protocol_fixture_start(&fixture));
    fixture.data->float_conf.hardware.leds.mode = (LedMode) 4;

    uint8_t v1_request[] = {101, COMMAND_INFO};
    vesc_if_fake_invoke_app_data_handler(v1_request, sizeof(v1_request));
    size_t len = 0;
    const uint8_t *payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    EXPECT_EQ_U32(len, 5u);
    EXPECT_EQ_U32(payload[4], LED_MODE_OFF);

    uint8_t v2_request[] = {101, COMMAND_INFO, 2, 0};
    vesc_if_fake_invoke_app_data_handler(v2_request, sizeof(v2_request));
    payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    EXPECT_EQ_U32(len, 60u);
    int32_t index = 55;
    EXPECT_EQ_U32(buffer_get_uint32(payload, &index) & 0x3u, 0u);
