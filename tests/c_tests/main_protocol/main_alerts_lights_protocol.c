...
static bool test_main_lights_control_protocol(void) {
    ...
    CHECK_U32(index, sizeof(set_lights));
    ...
    CHECK_U32(payload[1], MAIN_COMMAND_LIGHTS_CONTROL);
    CHECK_U32(payload[2], 0x01u);
...
static bool test_main_alerts_protocol(void) {
    ...
    CHECK_U32(len, 19u);
    CHECK_U32(payload[0], 101u);
    CHECK_U32(payload[1], MAIN_COMMAND_ALERTS_LIST);

    index = 2;
    CHECK_U32(buffer_get_uint32(payload, &index), 0x00000001u);
    CHECK_U32(buffer_get_uint32(payload, &index), 0x00000000u);
    CHECK_U32(payload[index++], FAULT_CODE_NONE);
    CHECK_U32(payload[index++], 1u);
    CHECK_U32(buffer_get_uint32(payload, &index), 1234u);
    CHECK_U32(payload[index++], ALERT_FW_FAULT);
    CHECK_U32(payload[index++], 1u);
    CHECK_U32(payload[index++], FAULT_CODE_NONE);
    CHECK_U32(index, len);
