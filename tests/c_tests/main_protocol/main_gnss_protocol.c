...
static uint32_t main_gnss_mask2(void) {
    return RT_MASK2_GNSS_LAT | RT_MASK2_GNSS_LON | RT_MASK2_GNSS_ALTITUDE | RT_MASK2_GNSS_SPEED |
...
static bool check_main_info_optional_gnss(bool missing_hook) {
    ...
    EXPECT_TRUE(main_gnss_invoke_without_signal(info_request, sizeof(info_request)));
    size_t len = 0;
...
static bool check_main_realtime_optional_gnss(bool missing_hook) {
    ...
    EXPECT_TRUE(main_gnss_invoke_without_signal(rt_request, sizeof(rt_request)));
    size_t len = 0;
    const uint8_t *payload = vesc_if_fake_last_app_data(&len);
    EXPECT_TRUE(payload != NULL);
    ...
    EXPECT_FLOAT_NEAR(buffer_get_float64_be(payload, &index), 0.0f);
    EXPECT_FLOAT_NEAR(buffer_get_float64_be(payload, &index), 0.0f);
    EXPECT_FLOAT_NEAR(buffer_get_float32_auto(payload, &index), 0.0f);
    EXPECT_FLOAT_NEAR(buffer_get_float32_auto(payload, &index), 0.0f);
    EXPECT_FLOAT_NEAR(buffer_get_float32_auto(payload, &index), 0.0f);
    EXPECT_EQ_U32(buffer_get_uint32(payload, &index), 0u);
    ...
    index = 7;
    buffer_append_uint32(rt_request, RT_MASK2_GNSS_ALTITUDE, &index);
    EXPECT_TRUE(main_gnss_invoke_without_signal(rt_request, sizeof(rt_request)));
...
static bool test_main_realtime_handles_optional_foc_id_unavailable(void) {
    MainGnssFixture fixture;
    EXPECT_TRUE(main_gnss_fixture_start(&fixture));
    // Older VESC C interfaces omit this optional hook; compatibility is the
    // behavior under test, not a firmware defect.
    fake_vesc_if.foc_get_id = NULL;

    uint8_t request[11] = {101, COMMAND_REALTIME_DATA, 0};
    int32_t index = 3;
    buffer_append_uint32(request, 0, &index);
    buffer_append_uint32(request, RT_MASK2_MOTOR_ID, &index);

    EXPECT_TRUE(main_gnss_invoke_without_signal(request, sizeof(request)));

    size_t len = 0;
    const uint8_t *payload = vesc_if_fake_last_app_data(&len);
    EXPECT_EQ_U32(len, 17u);
    index = 15;
    EXPECT_EQ_U32(buffer_get_uint16(payload, &index), 0u);
