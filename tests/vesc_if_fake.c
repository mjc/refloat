...
enum { FAKE_VESC_PACKAGE_DATA_OFFSET = 2036 };
...
#define FAKE_GETTER(type, name, field) type name(void) { return fake_state.field; }
...
#define FAKE_STATIC_GETTER(type, name, field) static type fake_##name(void) { return fake_state.field; }
...
#define FAKE_COUNT_CALL(name, count_field) \
    static void fake_##name(void) { ++fake_state.count_field; }
...
#define FAKE_SETTER(func, type, arg, field) \
    void vesc_if_fake_##func(type arg) { fake_state.field = arg; }
...
static lib_thread fake_spawn(void (*fun)(void *arg), size_t stack_size, const char *name, void *arg) {
    (void) fun;
    (void) stack_size;
    (void) name;
    (void) arg;
...
FAKE_RECORD_FLOAT(
    mc_set_current_off_delay,
    delay_sec,
    mc_set_current_off_delay_calls,
    last_current_off_delay
)
...
void vesc_if_fake_reset(void) {
    ...
    fake_vesc_if.should_terminate = fake_false;
    ...
    fake_vesc_if.mc_get_tot_current_directional_filtered = fake_mc_get_tot_current_directional_filtered;
...
void vesc_if_fake_set_data_buffer(uint32_t magic, uint8_t *buffer, size_t length) {
    FakeDataBufferInfo *info =
        (FakeDataBufferInfo *) ((uint8_t *) &fake_vesc_if + FAKE_VESC_PACKAGE_DATA_OFFSET);
    info->magic = magic;
    info->buffer = buffer;
    info->length = length;
}
