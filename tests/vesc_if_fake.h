...
void vesc_if_fake_set_imu_startup_done(bool done);
...
// Fake-input contract: Refloat config floats are fixed-point int16 values and
// VESC timer deltas are finite unsigned-counter durations. Test zero, negative,
// or out-of-range finite Refloat config values and zero dt. The VESC C API
// returns config ints as signed values, but VESC Tool constrains byte-serialized
// fields such as si_battery_cells to 1..255. Zero is only a malformed raw
// packet edge; negative values are not a useful VESC Tool bug input. VESC Tool
// does allow zero for si_gear_ratio and si_battery_ah; those are real boundary
// cases because BLDC can expose nonfinite speed/distance or battery level.
// VESC motor/app config floats are a separate boundary and can be malformed;
// use NaN/Inf there only to test the derived-value guards, not as VESC Tool
// configuration bugs. A BMS can_id of -1 is normal with no smart BMS and before
// the first sample. A NULL foc_get_id hook is normal on older VESC C interfaces.
// Other NaN/Inf inputs belong at raw IMU, malformed PPM, motor telemetry,
// VESC-derived battery data, or packet boundaries.
bool vesc_if_fake_set_cfg_float(CFG_PARAM param, float value);
void vesc_if_fake_set_cfg_int(CFG_PARAM param, int value);
void vesc_if_fake_set_eeprom_read_enabled(bool enabled);
void vesc_if_fake_set_eeprom_store_enabled(bool enabled);
void vesc_if_fake_set_eeprom_read_fail_call(size_t call_number);
void vesc_if_fake_set_eeprom_store_fail_call(size_t call_number);
void vesc_if_fake_set_eeprom_store_discard_call(size_t call_number);
void vesc_if_fake_set_eeprom_store_discard(bool discard);
void vesc_if_fake_set_odometer(uint64_t odometer);
